...
export function LoginPage() {
  ...
  const [deniedState, setDeniedState] = useState<{
    ...
    if (isExpiredRedirect) {
      return {
        reason: 'expired',
        message:
          'Tu periodo de acceso ha expirado. Tu sesión se ha cerrado automáticamente. Por favor contacta al administrador para renovar tu suscripción.',
      }
    }
    if (isBlockedRedirect) {
      return {
        reason: 'blocked',
        message:
          'Tu cuenta ha sido bloqueada. Por favor contacta al administrador para solicitar acceso nuevamente.',
      }
    }
    return null
  })

  async function performLogin(userToLogin = username, passToLogin = password) {
    setError('')
    setLoading(true)
    try {
  ...
  function handleBackToLogin() {
    ...
    setError('')
    setPassword('')
    if (searchParams.has('expired') || searchParams.has('blocked')) {
  ...
  return (
    <AuthSplitCard tagline="Sistema inteligente de gestión financiera personal para el control eficiente de ingresos, gastos y metas de ahorro.">
      {deniedState ? (
        <AccessDeniedCard
          reason={deniedState.reason}
          customMessage={deniedState.message}
          username={username}

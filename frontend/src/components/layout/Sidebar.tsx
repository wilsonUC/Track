...
function navClassName(isActive: boolean) {
  return `group flex w-full min-w-0 items-center gap-2.5 rounded-lg px-3 py-1.5 text-left text-sm transition ${
    isActive
      ? 'bg-indigo-500 font-semibold text-white dark:bg-slate-700'
      : 'text-indigo-100 hover:bg-indigo-600 dark:text-slate-300 dark:hover:bg-slate-800'
  }`
}
    ...
    <aside className="fixed inset-y-0 left-0 z-40 hidden h-dvh w-64 flex-col border-r border-transparent bg-indigo-700 p-4 text-indigo-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 md:flex">
      <div className="flex h-full min-h-0 flex-col">
        <div className="shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-indigo-200 dark:text-slate-400">Panel principal</p>
            </div>
          </div>
        </div>
          ...
          {visibleMenuItems.map((item) =>
            item.disabled ? (
              ...
              >
                {({ isActive }) => (
                  <>
                    <span className={`shrink-0 ${isActive ? 'text-white' : 'text-indigo-200 group-hover:text-indigo-50 dark:text-slate-400 dark:group-hover:text-slate-200'}`}>
                      <NavIcon section={item.id} />
                    </span>
                    <span className="min-w-0 flex-1 truncate">{item.label}</span>
                  </>
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `mt-4 block shrink-0 rounded-lg border p-3 transition ${
              isActive
                ? 'border-indigo-400 bg-indigo-600/80 ring-1 ring-indigo-300/50 dark:border-slate-600 dark:bg-slate-800 dark:ring-slate-600/50'
                : 'border-indigo-600/80 bg-indigo-800/40 hover:bg-indigo-800/70 dark:border-slate-700 dark:bg-slate-900 dark:hover:bg-slate-800'
            }`
          }
        ...
        <button
          type="button"
          onClick={onLogout}
          className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-indigo-500/80 px-3 py-2 text-sm font-medium text-indigo-100 transition hover:bg-indigo-600 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800"
        >

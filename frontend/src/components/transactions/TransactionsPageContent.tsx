...
import { deleteTransaction, fetchCategories, fetchTransactions, type ApiCategory } from '../../api/finanzas'
...
export function TransactionsPageContent({ variant }: TransactionsPageContentProps) {
  ...
  async function handleDelete(transaction: EnrichedTransaction) {
    const label = transaction.descripcion || 'este registro'
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'No se pudo eliminar la transacción.'
  ...
  return (
    <section className="space-y-4">
      ...
      <TransactionEditModal
        open={modal.open}
        mode={modal.open ? modal.mode : 'edit'}
        transaction={modal.open ? modal.transaction : null}
        categories={categories}

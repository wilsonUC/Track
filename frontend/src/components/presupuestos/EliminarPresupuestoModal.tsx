...
import {
  fetchInfoEliminacionPresupuesto,
  type InfoEliminacionPresupuesto,
} from '../../api/presupuestos'
import type { PresupuestoCardView } from './presupuestosTypes'

type EliminarPresupuestoModalProps = {
  open: boolean
  presupuesto: PresupuestoCardView | null
  onClose: () => void
  onConfirm: (id: number, modo?: 'eliminar_todo' | 'conservar_transacciones') => Promise<void>
}

export function EliminarPresupuestoModal({
  open,
  presupuesto,
  onClose,
  onConfirm,
}: EliminarPresupuestoModalProps) {
  const [loadingInfo, setLoadingInfo] = useState(false)
  const [info, setInfo] = useState<InfoEliminacionPresupuesto | null>(null)
  ...
  useEffect(() => {
    ...
    fetchInfoEliminacionPresupuesto(presupuesto.id)
      .then((data) => {
  ...
  const handleEliminar = async () => {
    ...
    setError('')
    try {
      await onConfirm(presupuesto.id, 'eliminar_todo')
      onClose()
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'No se pudo eliminar el presupuesto'
      setError(msg)
    } finally {
      setDeleting(false)
    }
        ...
        <div className="flex items-center justify-between border-b border-slate-100 p-5 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 truncate max-w-xs">{presupuesto.nombre}</p>
            </div>
          </div>
          <button
            type="button"
          ...
          {loadingInfo ? (
            ...
            <div className="space-y-3">
              <p className="text-sm text-slate-700 dark:text-slate-200">
                ¿Estás seguro de que deseas eliminar permanentemente el presupuesto{' '}
                <strong className="text-slate-900 dark:text-white">"{presupuesto.nombre}"</strong>?
              </p>
                ...
                <div className="rounded-xl border border-rose-200 bg-rose-50/70 p-3 text-xs text-rose-900 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-200">
                  <div className="flex items-start gap-2.5">
                    ...
                    <p className="text-[11px] leading-relaxed">
                      Este presupuesto tiene <strong>{info?.num_transacciones} {info?.num_transacciones === 1 ? 'gasto registrado' : 'gastos registrados'}</strong> (S/ {info?.total_monto.toFixed(2)}). Se eliminarán todos los gastos asociados y tu saldo se recalculará.
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-slate-400">
                  Esta acción eliminará el presupuesto de forma permanente.
                </p>
              )}
            </div>
          )}

...
type ReactivarPresupuestoModalProps = {
  open: boolean
...
export function ReactivarPresupuestoModal({
  open,
  presupuesto,
  mesInicial,
  saving,
  ...
  onClose,
  onConfirm,
}: ReactivarPresupuestoModalProps) {
  ...
  }, [open, presupuesto, mesInicial])

  if (!open || !presupuesto) return null

  const handleSubmit = (e: FormEvent) => {
    e.preventDefault()
    if (!fechaInicio || !limite) return
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-5">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div className="space-y-1 text-[11.5px] leading-relaxed">
                <p className="font-bold">
                  Inicia un nuevo ciclo limpio en S/ 0.00
                </p>
                <p className="text-emerald-800 dark:text-emerald-300/90">
                  Tus transacciones anteriores no se borran; se conservan seguras en tu historial general de gastos.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-100 pt-3 dark:border-slate-800">
            ...
            >
              ...
              <span>{saving ? 'Reactivando…' : 'Reactivar presupuesto'}</span>
            </button>
          </div>
        </form>
      </div>

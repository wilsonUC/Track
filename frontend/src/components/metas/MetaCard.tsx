...
export function MetaCard({ meta, onAsignar, onDesasignar, onEditar, onEliminar, onCambiarModo }: MetaCardProps) {
  ...
  const esBloqueadoPeriodo = estadoPeriodo === 'futuro' || estadoPeriodo === 'no_iniciado' || estadoPeriodo === 'finalizado'
  ...
  return (
    <article className="flex flex-col justify-between space-y-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all hover:shadow-md dark:border-slate-800/80 dark:bg-slate-900/90 dark:shadow-slate-950/40">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-1.5">
            ...
            {estadoPeriodo === 'futuro' ? (
              <span className="rounded-md border border-indigo-100 bg-indigo-50 px-2 py-0.5 text-[10px] font-black text-indigo-600 dark:border-indigo-900/50 dark:bg-indigo-950/40 dark:text-indigo-400">
                PRÓXIMAMENTE
              </span>
            ) : estadoPeriodo === 'no_iniciado' ? (
              <span className="rounded-md border border-slate-200 bg-slate-100 px-2 py-0.5 text-[10px] font-black text-slate-500 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-400">
                NO INICIADA
              </span>
      ...
      <div className="flex gap-2 pt-1">
        <button
          type="button"
          disabled={completada || esBloqueadoPeriodo}
          title={
            esBloqueadoPeriodo
              ? estadoPeriodo === 'futuro'
                ? 'Aún no disponible para este período futuro'
                : 'No activa en este período'
              : undefined
          }
          onClick={() => onAsignar(meta)}
          className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl border py-2.5 text-xs font-bold transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${
            esBloqueadoPeriodo
              ? 'border-slate-100 bg-slate-100 text-slate-400 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-500'
        ...
        >
          ...
          <span>{estadoPeriodo === 'futuro' ? 'Próximamente' : 'Asignar'}</span>
        </button>
        <button
          type="button"
          disabled={acumulado <= 0 || esBloqueadoPeriodo}
          title={
            esBloqueadoPeriodo
              ? estadoPeriodo === 'futuro'
                ? 'Aún no disponible para este período futuro'
          ...
          className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl border py-2.5 text-xs font-bold transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${
            esBloqueadoPeriodo
              ? 'border-slate-100 bg-slate-100 text-slate-400 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-500'
              : 'border-slate-100 bg-slate-50 text-slate-600 hover:bg-rose-50 hover:text-rose-600 dark:border-slate-800 dark:bg-slate-800/80 dark:text-slate-300 dark:hover:bg-rose-950/50'
          }`}
        >
